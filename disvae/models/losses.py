...
def _unpack_mu_logvar(latent_dist):
    """
    Accepts either:
      - tuple/list: (mu, logvar)
      - object with attributes .mu and .logvar  (e.g., SpatialGaussian)
    Returns (mu, logvar).
    """
    if isinstance(latent_dist, (tuple, list)):
        return latent_dist[0], latent_dist[1]
    # allows simple container classes
    if hasattr(latent_dist, "mu") and hasattr(latent_dist, "logvar"):
        return latent_dist.mu, latent_dist.logvar
    raise TypeError(
        "latent_dist must be (mu, logvar) or an object with .mu and .logvar"
    )
...
class BetaHLoss(BaseLoss):
    ...
    def __call__(self, data, recon_data, latent_dist, is_train, storer, **kwargs):
        ...
        mu, logvar = _unpack_mu_logvar(latent_dist)
        kl_loss = _kl_normal_loss(mu, logvar, storer)
...
class FactorKLoss(BaseLoss):
    ...
    def __call__(self, *args, **kwargs):
        if len(latent_dist[0].shape) != 2:
            raise ValueError(
                f"FactorKLoss currently only supports vector latents [B, D], "
                f"but got shape {latent_dist[0].shape}"
            )
        return super().__call__(latent_dist, *args, **kwargs)
...
class BtcvaeLoss(BaseLoss):
    ...
    def __call__(self, data, recon_batch, latent_dist, is_train, storer,
         ...
         # Fail fast if latent is not vector
        if len(latent_dist[0].shape) != 2:  # assuming latent_dist is (mu, logvar)
            raise ValueError(
                f"BtcvaeLoss currently only supports vector latents [B, D], "
                f"but got shape {latent_dist[0].shape}"
            )
        storer = self._pre_call(is_train, storer)
...
def _kl_normal_loss(mean, logvar, storer=None):
    ...
    reduce_dims = tuple(range(1, kl_elem.dim()))
    latent_kl_per_sample = kl_elem.sum(dim=reduce_dims)        # [B]
    total_kl = latent_kl_per_sample.mean()                     # scalar

    if storer is not None:
        storer['kl_loss'].append(total_kl.item())
        # Optional: per-dim logging only when vector to avoid huge logs
        if mean.dim() == 2:  # [B, D]
            latent_dim = mean.size(1)
            latent_kl = kl_elem.mean(dim=0)    # [D]
            for i in range(latent_dim):
                storer['kl_loss_' + str(i)].append(latent_kl[i].detach().cpu())
